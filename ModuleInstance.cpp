...
static BOOL IsToolHelpSupported()
{
	...
	hModToolHelp = ::LoadLibraryA("KERNEL32.DLL");
...
static BOOL IsPsapiSupported()
{
	...
	hModPSAPI = ::LoadLibraryA("PSAPI.DLL");
...
// Populate the module list using PSAPI or ToolHlp32
//---------------------------------------------------------------------------
BOOL CTaskManager::Populate(DWORD pid)
{
	m_pProcesses->ReleaseAll();
	return m_pLibHandler->PopulateProcessesByPID(pid); 
}
...
BOOL CPsapiHandler::PopulateProcessesByPID(DWORD pid)
{
	BOOL   bResult = TRUE;
	CExeModuleInstance* pProcessInfo;
...
BOOL CToolhelpHandler::PopulateProcessesByPID(DWORD pid)
{
	BOOL   bResult    = FALSE;
	CExeModuleInstance* pProcessInfo;
	HANDLE hSnapshot  = INVALID_HANDLE_VALUE;
