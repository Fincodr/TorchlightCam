...
CSystemTray::CSystemTray(CWnd* pParent, UINT uCallbackMessage, LPCTSTR szToolTip, 
						 HICON icon, UINT uID, MNUSTATUS* mnuStatus)
{
	Initialise();
	Create(pParent, uCallbackMessage, szToolTip, icon, uID, mnuStatus);
}
...
BOOL CSystemTray::Animate(UINT nDelayMilliSeconds, int nNumSeconds /*=-1*/)
{
	...
	m_nCurrentIcon = 0;
	//m_StartTime = 0; //COleDateTime::GetCurrentTime();
	m_nAnimationPeriod = nNumSeconds;
	m_hSavedIcon = GetIcon();

	// Setup a T1mer for the animation
	m_uIDT1mer = SetTimer(m_nIDEvent, nDelayMilliSeconds, NULL);
...
void CSystemTray::OnT1mer(UINT nIDEvent) 
{
	ASSERT(nIDEvent == m_nIDEvent);

	//COleDateTime CurrentTime = 0 //COleDateTime::GetCurrentTime();
